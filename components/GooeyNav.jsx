...
const GooeyNav = ({
  ...
  const handleClick = (e, index) => {
    ...
    // Navigate after starting the animation
    setTimeout(() => {
  ...
  // Sync active tab with current pathname (prefer exact match, then longest prefix)
  useEffect(() => {
    if (!pathname) return;

    // 1) Exact match
    let idx = items.findIndex((it) => it.href === pathname);

    // 2) Longest prefix match (ignore '/')
    if (idx === -1) {
      let bestLen = -1;
      items.forEach((it, i) => {
